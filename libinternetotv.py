# -*- coding: utf-8 -*-
# Source: https://github.com/Vytax/plugin.video.interneto.tv/blob/master/libinternetotv.py
import http.client
import re
import urllib
import sys
...
class InternetoTV(object):
    ...
    def getHTTP(self):
        if not self.HTTP:
            self.HTTP = http.client.HTTPConnection(self.HOST)
        return self.HTTP

    def getHTTPS(self):
        if not self.HTTPS:
            self.HTTPS = http.client.HTTPSConnection(self.HOST)
        return self.HTTPS
    ...
    def getCookie(self):
        ...
        c = self.getHTTPS()
        c.request("POST", "/prisijungti", loginData,
        ...
        if ITVAPP and ITVCOOKIE:
            self.COOKIE = 'ITVAPP=%s; ITVCOOKIE[remember_me]=%s' % (
                ITVAPP, ITVCOOKIE)
            return self.COOKIE
    ...
    def getChannelUrls(self, vid):
        ...
        videoData['description'] = epg_first.find(
            'div', class_='description').string
        videoData['mp4_hls'] = re.findall(
            ' \[\{type\: \'hls\', file\: \'([^\']*)\'', fdata, re.DOTALL)[0]
    ...
    def getVideoURL(self, url):
        ...
        c.request("GET", url, headers={
                  'Cookie': self.COOKIE, 'Accept-encoding': 'gzip'})
        response = c.getresponse()
        fdata = self.unzipResponse(response)
        ...
        if not videoData['mp4_hls'].startswith('http'):
            videoData['mp4_hls'] = re.findall(
                '\[\{sources\: \[\{file\: "([^"]*)"', fdata, re.DOTALL)[0]
